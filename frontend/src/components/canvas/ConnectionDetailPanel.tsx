...
export default function ConnectionDetailPanel({
	...
	return (
		<DetailPanel title="Connection" onClose={onClose} position="right-4 top-4">
			<DetailRow label="From:" value={hubs.get(a)?.name ?? "Unknown"} />
			<DetailRow label="To:" value={hubs.get(b)?.name ?? "Unknown"} />
			<DetailRow label="Capacity:" value={connection.capacity} />
			<DetailRow label="Active Drones:" value={activeDrones} />
		</DetailPanel>
	);
}
